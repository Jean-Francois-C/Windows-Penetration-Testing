...
It generates obfuscated Python scripts, designed to run with IronPython (https://ironpython.net/), which embed the original .NET payloads (exe) for stealthy execution.
...
#### USAGE
  - STEP 1. Compile and copy in the same directory all the (C#) .NET executable files that you want to pack (e.g. Rubeus.exe, Certify.exe, Seatbelt.exe) 
  ...
  - STEP 3. Generate obfuscated Python scripts that embed your (C#) .NET executable files.  
```
...
```
   - Alternatively, you may download and store the obfuscated Python scripts locally on disk prior to execution with IronPython. The obfuscation helps evade static analysis by most antivirus solutions, though this method may offer reduced stealth compared to in-memory execution.
```
Example:
--------
