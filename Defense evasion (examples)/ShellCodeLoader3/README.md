...
#### FEATURES
  ...
  - AMSI bypass method taken/adapted from the tools 'SharpKiller' and 'AMSI-Kill'
...
``` 
  ...
  - STEP 2. Encrypt the shellcode using AES-256 cypher encryption (in CBC mode) and then store it in a file (in Base64-encoded format)
 ```
Examples
[*] AES encryption - https://github.com/mvelazc0/defcon27_csharp_workshop/blob/master/Labs/lab4/3.cs
[*] Base64 encoding
...
``` 
  ...
  - STEP 6. Execution
```
[*] Command: loader.exe <domain name> <loader process name> <C:\filepath\base64-shellcode-file.txt> <AES key>
    Note:
    > Arg 1 = The first argument is the joined domain name of the target Windows machine (input for the sandbox evasion check)
    > Arg 2 = The second argument is the (process) name of the loader (e.g., "loader")
    > Arg 3 = The third argument is the path to the file containing your AES-256 encrypted shellcode encoded in base64 (i.e., C:\path\file.txt or .\path\file.txt)
    > Arg 4 = The fourth argument is the AES cypher passkey 
``` 
#### LICENSE
